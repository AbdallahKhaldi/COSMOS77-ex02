"""Initialise logging from ``config/logging_config.json`` (PRD_logging).

Applies the dictConfig, ensuring any file/FIFO handler's target directory
exists first. The project logs under the ``cosmos77_ex02`` namespace; the
FIFO handler (:class:`cosmos77_ex02.shared.fifo_handler.FifoLineRotatingHandler`)
writes JSON-lines so the whole run can be audited from ``logs/``.
"""
...
def init_logging(config_path: Path | str | None = None) -> None:
    """Apply the dictConfig found at ``config_path`` (defaults to repo config)."""
...
def get_logger(name: str = "cosmos77_ex02") -> logging.Logger:
    """Return a logger under the project's ``cosmos77_ex02.*`` namespace."""
